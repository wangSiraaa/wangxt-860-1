...
const seedData = async () => {
  try {
    ...
    const usersResult = await query(
      `INSERT INTO users (username, password, email, role, real_name, phone) VALUES 
    ...
    const projectsResult = await query(
      `INSERT INTO projects (name, code, customer_name, description, status, start_date, end_date, project_manager_id, created_by) VALUES 
       ...
       ON CONFLICT (code) DO NOTHING RETURNING id, name, code`,
    ...
    if (projectsResult.rows.length > 0) {
      project1Id = projectsResult.rows.find(p => p.code === 'PROJ001')?.id;
      project2Id = projectsResult.rows.find(p => p.code === 'PROJ002')?.id;
      project3Id = projectsResult.rows.find(p => p.code === 'PROJ003')?.id;
    } else {
      const existingProjects = await query('SELECT id, code FROM projects WHERE code IN ($1, $2, $3)', ['PROJ001', 'PROJ002', 'PROJ003']);
      project1Id = existingProjects.rows.find(p => p.code === 'PROJ001')?.id;
      project2Id = existingProjects.rows.find(p => p.code === 'PROJ002')?.id;
      project3Id = existingProjects.rows.find(p => p.code === 'PROJ003')?.id;
    }
    ...
    if (project1Id) {
      ...
      const milestonesResult = await query(
        `INSERT INTO milestones (project_id, name, description, planned_date, status, sequence, created_by) VALUES 
         ($1, '需求调研完成', '完成客户需求调研，输出需求规格说明书', '2024-01-31', 'completed', 1, $2),
         ($1, '系统配置完成', '完成系统基础配置和个性化设置', '2024-02-29', 'completed', 2, $2),
         ($1, '用户培训完成', '完成关键用户和最终用户培训', '2024-03-31', 'in_progress', 3, $2),
         ($1, '用户验收测试', '完成UAT测试并修复所有缺陷', '2024-04-30', 'pending', 4, $2),
         ($1, '系统上线', '生产环境部署和数据迁移', '2024-05-31', 'pending', 5, $2),
         ($1, '项目验收', '最终验收和项目结项', '2024-06-30', 'pending', 6, $2)
         ON CONFLICT (project_id, sequence) DO NOTHING RETURNING id, name, sequence`,
        [project1Id, adminId]
      );
      ...
      if (milestonesResult.rows.length > 0) {
        milestonesResult.rows.forEach(m => {
          milestoneIds[m.sequence] = m.id;
        });
      } else {
        const existingMilestones = await query('SELECT id, sequence FROM milestones WHERE project_id = $1 ORDER BY sequence', [project1Id]);
      ...
      if (milestoneIds[2] && milestoneIds[1]) {
        await query(
          `INSERT INTO milestone_dependencies (milestone_id, predecessor_milestone_id) VALUES 
           ($1, $2)
           ON CONFLICT DO NOTHING`,
      ...
      const risksResult = await query(
        `INSERT INTO risks (project_id, title, description, level, status, mitigation_measure, created_by) VALUES 
         ($1, '客户需求变更频繁', '客户业务部门多，需求变更可能性大，可能影响项目进度', 'high', 'open', '建立需求变更控制流程，严格执行CR审批', $2),
         ($1, '关键用户参与度不足', '客户方关键用户工作繁忙，参与项目时间有限', 'medium', 'mitigating', '提前与客户沟通，明确参与要求，设置激励机制', $2),
         ($1, '数据迁移质量风险', '历史数据格式复杂，数据清洗难度大', 'high', 'open', '制定详细的数据迁移方案，提前进行数据质量检查', $2),
         ($1, '集成接口稳定性', '与第三方系统集成，接口稳定性存在风险', 'medium', 'open', '进行充分的接口测试，准备降级方案', $2)
      ...
      const meetingsResult = await query(
        `INSERT INTO meeting_minutes (project_id, title, meeting_date, location, content, attendees, created_by) VALUES 
         ($1, '项目启动会', '2024-01-05', '客户现场会议室A', '项目启动，介绍项目目标、范围、团队组成和工作计划', '张三,李四,王五', $2),
         ($1, '需求调研讨论会', '2024-01-15', '线上会议', '讨论业务流程和功能需求，确认需求范围', '张三,李四', $2),
         ($1, '系统配置评审会', '2024-02-25', '客户现场会议室B', '评审系统配置方案，确认参数设置', '张三,王五,赵六', $2)
         ON CONFLICT DO NOTHING RETURNING id`,
        [project1Id, adminId]
      );
      ...
      if (milestoneIds[1]) {
        const acceptanceResult = await query(
          `INSERT INTO acceptance_forms (project_id, milestone_id, title, content, status, submitted_by, submitted_at) VALUES 
           ($1, $2, '需求调研验收单', '需求调研工作已完成，输出《需求规格说明书V1.0》，经双方确认无误。', 'accepted', $3, '2024-01-31 10:00:00')
           ON CONFLICT DO NOTHING RETURNING id`,
